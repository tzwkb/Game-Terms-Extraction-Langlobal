...
# ═══════════════════════════════════════════════════════════
    ...
    if col_info:
        ...
        if eff_src:
            ...
            if cfg.bilingual:
                st.selectbox("原文英文列", range(len(src_headers)),
                             index=min(st.session_state.get("_src_en_col", 1), len(src_headers) - 1),
                             format_func=lambda i: f"[{i}] {src_headers[i]}",
                             key="_src_en_col")
            cfg.no_translate = st.checkbox(
                "跳过翻译（仅提取术语，不翻译）",
                ...
                try:
                    ...
                    if data.get("chunk_idx", 0) > 0:
                        ...
                        all_ckpts.append({
                            ...
                            "src_en_col": meta.get("src_en_col", 1),
                            "no_translate": meta.get("no_translate", False),
            ...
            for c in all_ckpts:
                ...
                with col_c2:
                    if st.button("继续", key=f"resume_{c['name']}", disabled=not has_files, use_container_width=True):
                        cfg.bilingual = c.get("bilingual", False)
                        cfg.no_translate = c.get("no_translate", False)
                        task = ProcessingTask()
